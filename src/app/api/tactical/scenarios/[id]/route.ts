...
import { deleteTacticalScenarioBundle, loadTacticalScenarioBundle, saveTacticalScenarioBundle, TacticalScenarioFileError } from "@/plugins/characterCombat/server/tacticalScenarioFiles";
...
export async function DELETE(
  _request: Request,
  context: RouteContext<"/api/tactical/scenarios/[id]">,
) {
  try {
    const { id } = await context.params;
    const deleted = await deleteTacticalScenarioBundle(id);
    return NextResponse.json({ deleted });
  } catch (error) {
    if (error instanceof TacticalScenarioFileError) {
      return NextResponse.json(
    ...
    return NextResponse.json({
      error: error instanceof Error
        ? error.message
        : "Scenario file operation failed.",
    }, { status: 500 });
  }
}
