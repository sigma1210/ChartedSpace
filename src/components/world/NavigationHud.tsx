...
import {
  selectEffectiveJumpDestination,
  selectEffectiveJumpDestinationKey,
  selectEffectivePlotStatus,
  selectJumpRangeCells,
...
export const NavigationHudContent = () => {
  ...
  const cells = useAppSelector(selectJumpRangeCells);
  const targets = useAppSelector(selectJumpRangeTargets);
  const selectedKey = useAppSelector(selectEffectiveJumpDestinationKey);
  const selectedDestination = useAppSelector(selectEffectiveJumpDestination);
  const loading = useAppSelector(selectJumpTargetsLoading);
  ...
  return (
    <NavigationHud
      ...
      onSelect={(target) => dispatch(selectJumpDestination(target))}
      onPlotCourse={() => dispatch(plotJumpCourse())}
      onExecuteJump={() => {
        if (!selectedDestination) return;
        dispatch(executePlottedJump(selectedDestination));
      }}
    />
  );
};
