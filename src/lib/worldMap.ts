  ...
  for (let id = 0; id < 20; id++) {
    ...
    if (isUp) {
      for (let i = 0; i < hexPerSide; i++) {
        for (let j = 0; j <= i; j++) {
          hexes.push({
            left: left + j * HEX_W + (hexPerSide - i) * 16,
            top: top + i * ROW_H,
            triangleId: id,
            isPolar: isSmall,
            terrain: "land",
...
export const assignTerrain = (
    ...
    while (ocean.size < target && front.length > 0) {
      ...
      for (let n = 0; n < cells.length; n++) {
        ...
        if (!ocean.has(n)) {
          ...
          if (dx <= HEX_W * 1.5 && dy <= ROW_H * 1.5 && rand() < 0.7) {
            ocean.add(n);
            front.push(n);
          }
        }
  ...
  if (isFr) {
    ...
    cells.forEach((h) => {
      ...
      else if (h.terrain === "ocean" || h.terrain === "fluid")
        h.terrain = "ice";
    });
    return cells;
  }
