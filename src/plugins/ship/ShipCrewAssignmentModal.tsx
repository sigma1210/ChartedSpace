...
export const ShipCrewAssignmentModal = () => {
  const dispatch = useAppDispatch();
  const activeModal = useAppSelector(selectActiveModal);
  const visible = activeModal === "shipCrewAssignment";
  ...
  return (
    ...
    >
      <div
        className="hud-panel flex h-[min(88vh,760px)] w-[min(92vw,1040px)] flex-col overflow-hidden border-white/22 shadow-[inset_0_1px_0_rgba(255,255,255,0.20),inset_0_-1px_0_rgba(255,255,255,0.06),0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl"
      ...
      >
        <div className="hud-panel-header flex shrink-0 items-center justify-between gap-4 border-white/18 px-4 py-2">
          ...
          <button
            type="button"
            onClick={() => dispatch(closeModal())}
            aria-label="Close ship crew assignment"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-4 py-3">
          <ShipCrewAssignmentHudContent presentation="modal" />
        </div>
      </div>
    </div>
  );
...
export default ShipCrewAssignmentModal;
