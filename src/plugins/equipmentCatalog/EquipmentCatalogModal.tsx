...
import { EquipmentCatalogHudContent } from "./EquipmentCatalogHud";

export const EquipmentCatalogModal = () => {
  const dispatch = useAppDispatch();
  const activeModal = useAppSelector(selectActiveModal);
  const visible = activeModal === "equipmentCatalog";
  ...
  return (
    ...
    >
      <div
        className="hud-panel flex h-[min(86vh,760px)] w-[min(94vw,560px)] flex-col overflow-hidden border-white/22 shadow-[inset_0_1px_0_rgba(255,255,255,0.20),inset_0_-1px_0_rgba(255,255,255,0.06),0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl"
      ...
      >
        <div className="hud-panel-header flex shrink-0 items-center justify-between gap-4 border-white/18 px-4 py-2">
          ...
          <button
            type="button"
            onClick={() => dispatch(closeModal())}
            aria-label="Close equipment catalog"
            className="text-(--hud-text-dim) transition-colors hover:text-(--hud-text)"
          >
            <X size={14} strokeWidth={1.5} />
          </button>
        </div>
        <div className="flex min-h-0 flex-1 items-start justify-center overflow-hidden px-3 py-2">
          {visible && <EquipmentCatalogHudContent />}
        </div>
      </div>
    </div>
  );
