...
export interface Combatant { id: string; name: string; side: CombatSide; sourceCrewId?: string; sourceCharacterId?: string | null; reinforcementTurn?: number; position: GridPoint; facing: "north" | "east" | "south" | "west"; health: number; defeated: boolean; surrendered: boolean; weapon: WeaponProfile; weaponSkill: number; meleeWeapon: { name: string; penetration: number }; meleeRating: number; armor: number; armorName?: string; grenades: number; medkits: number; breachingCharges?: number; vaccSuit?: boolean; woundState: WoundState }
export interface CombatScenario {
  id: string; title: string; briefing: string; objective: string; width: number; height: number;
  walls: WallSegment[]; doors: DoorSegment[]; objects: MapObject[]; combatants: Combatant[]; gravityMode?: "normal" | "zero-g"; handholds?: GridPoint[]; vacuumSources?: GridPoint[]; victoryCondition?: "secure-objective" | "rescue-extract" | "hold-zone" | "staged-objectives"; captiveId?: string; holdUntilTurn?: number; stageObjectiveIds?: string[]; stageUnlockDoorId?: string;
}
