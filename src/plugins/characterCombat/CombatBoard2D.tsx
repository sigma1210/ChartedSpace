...
import { previewCoveringFire, previewDoorCoverage, previewOverwatch, previewStructuralTarget } from "./slice";
...
export const CombatBoard2D = ({ scenario, currentTurn, selectedCombatantId, lockedTargetId, coveringCombatantIds, overwatchCombatantIds, onSelectCombatant, onSelectTarget, onClearSelection, reachableKeys, validTargetIds, coveredTargetIds, grenadeTargetKeys, grenadeBlastKeys, grenadeTargeting, plannedMove, hoveredDestination, onPreviewMove, onPreviewGrenade, onHoverDestination }: {
  ...
  const lastWeaponImpact = useAppSelector((state) => state.plugins.characterCombat.lastWeaponImpact);
  ...
  <svg viewBox={`-20 -20 ${scenario.width * cell + 40} ${scenario.height * cell + 40}`} className="h-full w-full" role="img" aria-label="Two dimensional boarding action deck plan" onClick={onClearSelection}>
    ...
    {Array.from({ length: scenario.width }, (_, x) => Array.from({ length: scenario.height }, (_, y) => {
      ...
      const inAttackField = attackFieldKeys.has(pointKey(point));
    ...
    {lastWeaponImpact && <g pointerEvents="none">
      <circle cx={(lastWeaponImpact.point.x + 0.5) * cell} cy={(lastWeaponImpact.point.y + 0.5) * cell} r="21" fill={!lastWeaponImpact.hit ? "#334155" : lastWeaponImpact.ammunitionKind === "he" ? "#dc2626" : "#7c3aed"} stroke={lastWeaponImpact.hit ? "#fef08a" : "#f8fafc"} strokeWidth="5" strokeDasharray={lastWeaponImpact.hit ? undefined : "6 4"} />
      <text x={(lastWeaponImpact.point.x + 0.5) * cell} y={(lastWeaponImpact.point.y + 0.5) * cell + 4} textAnchor="middle" fill="white" fontSize="9" fontWeight="bold" fontFamily="monospace">{lastWeaponImpact.hit ? lastWeaponImpact.ammunitionLabel : "MISS"}</text>
    </g>}
    ...
    {scenario.objects.map((object) => {
      ...
      return object.kind !== "cover" ? (
        ...
        <g key={object.id} className={structuralTargeting ? "cursor-crosshair" : undefined} onClick={(event) => { if (structuralTargeting) { event.stopPropagation(); dispatch(previewStructuralTarget(object.id)); } }}>
          <rect x={x - 23} y={y - 20} width="46" height="40" rx="3" fill={structuralDamageById?.[object.id] ? "#7c2d12" : "#5b4636"} stroke={plannedStructuralTargetId === object.id ? "#ffffff" : structuralTargeting ? "#e879f9" : structuralDamageById?.[object.id] ? "#fb923c" : "#d6a56d"} strokeWidth={structuralTargeting ? "5" : "2"} />
          <path d={`M ${x - 23} ${y} h 46 M ${x} ${y - 20} v 40`} stroke="#d6a56d" strokeWidth="2" />
        </g>
      );
    })}

    {scenario.walls.map((wall) => <line key={wall.id} x1={wall.from.x * cell} y1={wall.from.y * cell} x2={wall.to.x * cell} y2={wall.to.y * cell} stroke={plannedStructuralTargetId === wall.id ? "#ffffff" : structuralTargeting ? "#e879f9" : structuralDamageById?.[wall.id] ? "#fb923c" : "#d5e3ea"} strokeWidth={structuralTargeting ? "16" : "10"} strokeLinecap="round" className={structuralTargeting ? "cursor-crosshair" : undefined} onClick={(event) => { if (structuralTargeting) { event.stopPropagation(); dispatch(previewStructuralTarget(wall.id)); } }} />)}
    {scenario.doors.map((door) => (
      <g key={door.id} pointerEvents={doorCoverTargetIds.has(door.id) || structuralTargeting ? "auto" : "none"} className={doorCoverTargetIds.has(door.id) || structuralTargeting ? "cursor-pointer" : ""} onClick={(event) => { if (structuralTargeting && !door.open) { event.stopPropagation(); dispatch(previewStructuralTarget(door.id)); } else if (doorCoverTargetIds.has(door.id)) { event.stopPropagation(); dispatch(previewDoorCoverage(door.id)); } }}>
        {!door.open && <line x1={door.from.x * cell} y1={door.from.y * cell} x2={door.to.x * cell} y2={door.to.y * cell} stroke={plannedStructuralTargetId === door.id ? "#ffffff" : structuralTargeting ? "#e879f9" : structuralDamageById?.[door.id] ? "#fb923c" : Object.values(coveredDoorByCombatantId ?? {}).includes(door.id) ? "#fde047" : "#f59e0b"} strokeWidth={Object.values(coveredDoorByCombatantId ?? {}).includes(door.id) ? "18" : "12"} />}
    ...
    {scenario.combatants.filter((unit) => (!unit.reinforcementTurn || unit.reinforcementTurn <= currentTurn) && (unit.side === "player" || (observedEnemyIds ?? []).includes(unit.id))).map((unit) => {
      ...
      return (
        <g key={unit.id} opacity={active ? 1 : 0.5} pointerEvents={active ? "auto" : "none"} onClick={(event) => { if ((player && active) || validTarget) event.stopPropagation(); if (player && active) onSelectCombatant(unit.id); else if (validTarget) onSelectTarget(unit.id); }} className={(player && active) || validTarget ? "cursor-pointer" : "cursor-default"}>
          ...
          {coveredTarget && <g aria-label="Target in cover" transform={`translate(${x - 31} ${y + 11})`}><path d="M 0 -8 L 7 -5 L 6 2 Q 5 8 0 11 Q -5 8 -6 2 L -7 -5 Z" fill="#78350f" stroke="#fbbf24" strokeWidth="2" /><path d="M -3 0 L -1 3 L 4 -3" fill="none" stroke="#fef3c7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></g>}
