...
export interface SnapShotResult { hit: boolean; hitRoll: number; hitModifier: number; hitTotal: number; targetNumber: number; range: number; rangeBand: WeaponRangeBand; weaponAccuracy: number; weaponPenetration: number; attackArc: AttackArc; arcModifier: number; evadeModifier: number; woundRoll: number | null; woundTotal: number | null; cover: number; woundState: WoundState }
export interface MeleeResult { roll: number; modifier: number; total: number; attackArc: AttackArc; arcModifier: number; woundState: WoundState }
...
export const resolveMelee = (attacker: Combatant, target: Combatant, die: number, attackerSuppressed = false): MeleeResult => {
  ...
  const modifier = attacker.meleeRating - target.meleeRating + attacker.meleeWeapon.penetration - target.armor - (attacker.woundState === "light" ? 1 : 0) + arcModifier - (attackerSuppressed ? 1 : 0);
  const total = die + modifier;
  const woundState: WoundState = total <= 2 ? "healthy" : total <= 4 ? "light" : total === 5 ? "serious" : total === 6 ? "unconscious" : "dead";
  return { roll: die, modifier, total, attackArc, arcModifier, woundState };
};

export const resolveSnapShot = (attacker: Combatant, target: Combatant, hitDice: DicePair, woundDice: DicePair, cover = 0, fireMode: FireMode = "snap", targetEvading = false, attackerSuppressed = false): SnapShotResult | null => {
  ...
  const hitModifier = attacker.weaponSkill + weaponAccuracy - (attacker.woundState === "light" ? 1 : 0) - (fireMode === "snap" ? 1 : 0) - (fireMode === "covering" ? 2 : 0) + (fireMode === "automatic" ? 4 : 0) + arcModifier + evadeModifier - (attackerSuppressed ? 1 : 0);
  const hitTotal = hitRoll + hitModifier;
  if (hitTotal < profile.targetNumber) return { ...profile, hit: false, hitRoll, hitModifier, hitTotal, weaponAccuracy, weaponPenetration, attackArc, arcModifier, evadeModifier, woundRoll: null, woundTotal: null, cover, woundState: target.woundState };
  const woundRoll = woundDice.first + woundDice.second;
  const woundTotal = woundRoll + weaponPenetration - target.armor - cover;
  return { ...profile, hit: true, hitRoll, hitModifier, hitTotal, weaponAccuracy, weaponPenetration, attackArc, arcModifier, evadeModifier, woundRoll, woundTotal, cover, woundState: woundStateForTotal(woundTotal) };
};
