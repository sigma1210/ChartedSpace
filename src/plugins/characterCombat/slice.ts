...
const tacticalPathObservedByCrew = (map: TacticalMapState, points: GridPoint[]) => map.scenario.combatants
...
const recordTacticalObservedEvent = (map: TacticalMapState, unit: CombatScenario["combatants"][number], event: string, points: GridPoint[] = [unit.position]) => {
...
const recordTacticalMovementAnimation = (map: TacticalMapState, unit: CombatScenario["combatants"][number], origin: GridPoint, path: GridPoint[], mode: "walk" | "run") => {
...
const freshTacticalMap = (entries: readonly TacticalCrewInput[], loadoutIds: readonly ArmoryLoadoutId[], layouts?: Pick<TacticalMapState, "characterHudLayout" | "enemyHudLayout" | "actionHudLayout" | "characterInformationHudLayout" | "eventsHudLayout" | "scenarioHudLayout" | "deploymentHudLayout">, options: { setup?: boolean; lightingPreset?: TacticalLightingPreset; definition?: TacticalScenarioDefinitionFile; consoleVictory?: TacticalConsoleVictoryDefinitionFile } = {}): TacticalMapState => {
  ...
  return {
    ...
    deploymentHudLayout: layouts?.deploymentHudLayout ?? { visible: true, pinned: false, position: { x: 16, y: 190 } },
    ...
    events: [],
    turn: 1,
    actionPointsByCharacterId: Object.fromEntries(scenario.combatants.map((unit) => [unit.id, options.setup || unit.defeated ? 0 : 6])),
    actedCharacterIds: [],
...
const resolveTacticalSatchelCharge = (map: TacticalMapState, chargeId: string, rolls: TacticalCollateralRolls = {}) => {
...
const detonateTacticalSatchelsReceivingCollateral = (map: TacticalMapState, center: GridPoint, penetration: number, rolls: TacticalCollateralRolls = {}) => {
...
const slice = createSlice({ name: "characterCombat", initialState: initialCharacterCombatState, reducers: {
  initializeTacticalMap: (state, action: PayloadAction<TacticalCrewInput[]>) => {
    const layouts = state.tacticalMap ? { characterHudLayout: state.tacticalMap.characterHudLayout, enemyHudLayout: state.tacticalMap.enemyHudLayout, actionHudLayout: state.tacticalMap.actionHudLayout, deploymentHudLayout: state.tacticalMap.deploymentHudLayout, characterInformationHudLayout: state.tacticalMap.characterInformationHudLayout, eventsHudLayout: state.tacticalMap.eventsHudLayout, scenarioHudLayout: state.tacticalMap.scenarioHudLayout } : undefined;
    state.tacticalMap = freshTacticalMap(action.payload, state.extendedArmoryLoadoutIds ?? state.armoryLoadoutIds, layouts);
  },
  initializeTacticalMapSetup: (state, action: PayloadAction<TacticalCrewInput[]>) => {
    const layouts = state.tacticalMap ? { characterHudLayout: state.tacticalMap.characterHudLayout, enemyHudLayout: state.tacticalMap.enemyHudLayout, actionHudLayout: state.tacticalMap.actionHudLayout, deploymentHudLayout: state.tacticalMap.deploymentHudLayout, characterInformationHudLayout: state.tacticalMap.characterInformationHudLayout, eventsHudLayout: state.tacticalMap.eventsHudLayout, scenarioHudLayout: state.tacticalMap.scenarioHudLayout } : undefined;
  ...
  setTacticalTerrainLights: (state, action: PayloadAction<boolean>) => {
    const map = state.tacticalMap;
    if (!map || map.scenarioStatus !== "setup") return;
    (map.scenario.lightSources ?? []).forEach((source) => { source.on = action.payload; });
    map.visibleHostileIdsAtPhaseStartByCombatantId = tacticalVisibilitySnapshot(map.scenario);
  },
  recordTacticalExploration: (state, action: PayloadAction<string[]>) => {
    const map = state.tacticalMap;
    if (!map) return;
    map.exploredCellKeys = [...new Set([...(map.exploredCellKeys ?? []), ...action.payload])];
  },
  recordTacticalEnemySightings: (state, action: PayloadAction<{ visibleCellKeys: string[]; enemies: { id: string; position: GridPoint }[] }>) => {
    const map = state.tacticalMap;
    if (!map) return;
  ...
  rotateTacticalDeploymentCharacter: (state, action: PayloadAction<"left" | "right">) => {
    const map = state.tacticalMap;
    const unit = map ? tacticalCombatant(map, map.deploymentCharacterId) : null;
  ...
  setTacticalDeploymentPosture: (state, action: PayloadAction<"standing" | "prone">) => {
    const map = state.tacticalMap;
    const unit = map ? tacticalCombatant(map, map.deploymentCharacterId) : null;
    if (!map || map.scenarioStatus !== "setup" || unit?.side !== "player" || !(map.deployedCharacterIds ?? []).includes(unit.id)) return;
  ...
  updateTacticalDeploymentHud: (state, action: PayloadAction<CharacterCombatHudLayout>) => {
  ...
  previewTacticalExtinguishFire: (state, action: PayloadAction<GridPoint>) => {
    const map = state.tacticalMap;
    const characterId = map?.activeCharacterId;
    const character = map ? tacticalCombatant(map, characterId) : null;
  ...
  confirmTacticalExtinguishFire: (state) => {
    const map = state.tacticalMap;
    const characterId = map?.activeCharacterId;
    const character = map ? tacticalCombatant(map, characterId) : null;
  ...
  cancelTacticalExtinguishFire: (state) => {
    const map = state.tacticalMap;
    if (!map) return;
    map.plannedExtinguishFire = null;
    map.movementMode = "walk";
  },
  beginTacticalSatchelPlacement: (state) => {
    const map = state.tacticalMap;
    const characterId = map?.activeCharacterId;
    const character = map ? tacticalCombatant(map, characterId) : null;
  ...
  confirmTacticalSatchelPlacement: (state) => {
    const map = state.tacticalMap;
    const characterId = map?.activeCharacterId;
    const character = map ? tacticalCombatant(map, characterId) : null;
  ...
  cancelTacticalSatchelPlacement: (state) => {
    if (!state.tacticalMap) return;
    state.tacticalMap.satchelPlacementPending = false;
    state.tacticalMap.movementMode = "walk";
  },
  detonateTacticalSatchelCharge: (state, action: PayloadAction<{ chargeId: string; rollsByCombatantId: TacticalCollateralRolls }>) => {
    const map = state.tacticalMap;
    const characterId = map?.activeCharacterId;
    const character = map ? tacticalCombatant(map, characterId) : null;
  ...
  defuseTacticalSatchelCharge: (state, action: PayloadAction<string>) => {
    const map = state.tacticalMap;
    const characterId = map?.activeCharacterId;
    const character = map ? tacticalCombatant(map, characterId) : null;
  ...
  resolveTacticalAdjacencyReaction: (state, action: PayloadAction<{ fire: boolean; hitDice: DicePair; woundDice: DicePair }>) => {
    ...
    if (action.payload.fire
      ...
      const result = resolveSnapShot(defender, mover, action.payload.hitDice, action.payload.woundDice, coverProtection(map.scenario, defender.id, mover.id), "snap", map.evadingCombatantIds.includes(mover.id), map.suppressedCombatantIds.includes(defender.id), map.bracedCombatantIds.includes(defender.id), visibility.darknessModifier);
      if (result) {
  ...
  runTacticalEnemyPhase: (state, action: PayloadAction<TacticalEnemyPhaseRolls>) => {
    ...
    for (const enemy of enemies) {
      ...
      const meleeTarget = meleeEnemies(scenario, enemy.id).find((unit) => unit.side === "player") ?? null;
      if (meleeTarget) {
        ...
        const returnEligible = meleeEnemies(scenario, meleeTarget.id).some((unit) => unit.id === enemy.id);
      ...
      let attackTarget: CombatScenario["combatants"][number] | null = tacticalRangedEnemies(scenario, enemy.id)
      ...
      if (attackTarget && !shouldMoveCloser) {
        ...
        const suppressionTarget = map.actionPointsByCharacterId[enemy.id] >= 6 && enemy.weapon.automatic && ammunition >= 3 ? tacticalRangedEnemies(scenario, enemy.id)
  ...
  confirmTacticalMove: (state, action: PayloadAction<{ moraleDice: DicePair; snapDice: { hitDice: DicePair; woundDice: DicePair }; enemyReactionRolls?: Record<string, { hitDice: DicePair; woundDice: DicePair }>; meleeDice?: { attackRoll: number; responseRoll: number } } | undefined>) => {
    ...
    if (character && adjacencyEntry && action.payload?.moraleDice && character.moraleFactor !== undefined) {
      const leadershipModifier = Object.entries(map.movingAdjacentMoraleResultByLeaderId).reduce((total, [leaderId, passed]) => {
        const leader = tacticalCombatant(map, leaderId);
        return leader && leader.id !== character.id && leader.side === character.side && !leader.defeated && hasLineOfSight(map.scenario, leader.position, character.position)
          ? total + (passed ? 1 : -1) * Math.max(0, leader.leadershipRating ?? 0)
          : total;
      }, 0);
      const morale = resolveAhlMoraleCheck({ moraleFactor: character.moraleFactor, woundState: character.woundState }, action.payload.moraleDice, leadershipModifier);
    ...
    if (character && action.payload?.enemyReactionRolls) {
      ...
      for (const { enemy, stepIndex } of reactions) {
        ...
        const result = resolveSnapShot(enemy, character, dice.hitDice, dice.woundDice, coverProtection(map.scenario, enemy.id, character.id), "snap", mode === "evade", map.suppressedCombatantIds.includes(enemy.id), map.bracedCombatantIds.includes(enemy.id), visibility.darknessModifier);
        if (!result) continue;
        map.actionPointsByCharacterId[enemy.id] -= 3;
        spendTacticalAmmunition(map, enemy, 1);
  ...
  previewBreachDoor: (state, action: PayloadAction<string>) => {
    ...
    if (!id || !unit || !door || state.placedBreachingChargeByDoorId[door.id] || state.status !== "active" || state.actedCombatantIds.includes(id) || state.draggingCombatantByCarrierId[id] || (state.actionPointsById[id] ?? 0) < 3 || (unit.breachingCharges ?? 0) < 1) return;
  ...
  confirmBreachDoor: (state) => {
    ...
    if (!scenario || !id || !unit || !door || state.placedBreachingChargeByDoorId[door.id] || !breachableDoorsAdjacentTo(scenario, id).some((candidate) => candidate.id === door.id) || (state.actionPointsById[id] ?? 0) < 3 || (unit.breachingCharges ?? 0) < 1) return;
    state.placedBreachingChargeByDoorId[door.id] = id;
    unit.breachingCharges = (unit.breachingCharges ?? 0) - 1;
    state.actionPointsById[id] -= 3;
    if (state.actionPointsById[id] === 0) state.actedCombatantIds.push(id);
